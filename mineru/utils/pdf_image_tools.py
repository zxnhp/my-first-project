...
import requests
import json
class FileBasedRequestWriter:
    def __init__(self) -> None:
        # 你可以把上传url和header写成属性方便修改
        self.upload_url = "http://dev-api.sieiot.com/aigc/backend/iot-base-attachment/single/file-upload"
        self.headers = {
            "Authorization": "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1ODU1ODM4Yy00OGVkLTRlNzMtYmNmNi1jZDU1MWZlYjZkNmYiLCJpc3MiOiJzaWUtaW90LWp3dCIsInN1YiI6IntcInBob25lTnVtYmVyXCI6XCIxMzYyNjIzMjEwOFwiLFwibG9naW5Gcm9tXCI6XCJQQ1wiLFwidXNlcm5hbWVcIjpcIjEzNjI2MjMyMTA4XCJ9IiwiaWF0IjoxNzUwMjk0NzE2fQ._1AHaRPeDWhsqb1IkwT17dNiMmgnC9UHidgzZPWTmS4",
        }

    def write(self,_,data: bytes) -> None:
        # 拼接路径（如果是相对路径，和parent_dir拼）
        ...
        print(res_json.get("data", {}).get("filePath"))
        return res_json.get("data", {}).get("filePath")
...
def cut_image(bbox: tuple, page_num: int, page_pil_img, return_path, image_writer: FileBasedDataWriter, scale=2):
    ...
    image_url = image_writer.write(img_hash256_path, img_bytes)
